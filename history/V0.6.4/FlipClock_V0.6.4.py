# -*- coding: utf-8 -*-
"""
鐵路翻牌時鐘 FlipClock V0.6.4

Windows Nuitka 專案參數：
直接執行「python -m nuitka FlipClock_V0.6.4.py」即可產生單一 EXE。
"""
...
#    nuitka-project: --output-filename=FlipClock_V0.6.4.exe
#    nuitka-project: --remove-output
#    nuitka-project: --assume-yes-for-downloads
#    nuitka-project: --python-flag=no_docstrings
#    nuitka-project: --file-version=0.6.4.0
#    nuitka-project: --product-version=0.6.4.0
#    nuitka-project: --product-name=FlipClock
#    nuitka-project: --file-description=Railway Flip Clock
#    nuitka-project: --report={MAIN_DIRECTORY}/build/FlipClock_V0.6.4_compilation-report.xml
...
class NativeWindowsTray:
    ...
    def _run_windows(self) -> None:
        ...
        if extracted and (small_icon or large_icon):
            hicon = small_icon if small_icon else large_icon
        else:
            hicon = user32.LoadIconW(
                None,
                ctypes.c_void_p(self.IDI_APPLICATION),
            )
...
class RailwayFlipClock(tk.Toplevel):
    ...
    def show_from_tray(self) -> None:
        if self.is_closing or not self.tray_available:
            return
